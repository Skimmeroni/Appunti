...
One way to obtain a graphical representation for two variables is simply
to extend bar plots to three dimensions (the values for the first character
on the $x$ axis, the values for the second character on the $y$ axis and
the frequencies on the $z$ axis). An alternative representation is the
*heat map*: a table where the possible values for the two variables are
arranged on the sides and each $(v_(j), v_(i))$ cell is coloured: said
colour is as intense as the value of $f_(j, i)$.

#exercise[
	Suppose that a sample has been collected regarding the number of rooms
	in a flat and the number of people inhabiting such flat:
	...
	#align(
		...
		table(
			columns: 18,
			stroke: none,
			..csv("correlation_data.csv", delimiter: " ").flatten()
		)
	)

	Compute the four frequencies for the sample and draw a heat map for the
	absolute frequency.
]
#solution[
	#align(
		...
		table(
			...
			..csv("correlation_result.csv").flatten()
		)
	)

	#figure(image("correlation_result.svg", width: 66%))
]
