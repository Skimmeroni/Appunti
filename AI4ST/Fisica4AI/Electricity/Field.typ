...
#figure(
	caption: [The electric field lines of two charges. Each line
...
#figure(
	caption: [Strength of the electric field with respect to the value of the $z$ coordinate.],
	[#image("ring.svg", width: 75%)]
)
...
#figure(
	caption: [An electrically charged disk of length $r$, whose charge is
	          ...
	          point on the $i$-th ring. The electric field can either point
	          right (blue) or left (red) depending on the sign of the source
	          charge.],
	[#cetz.canvas({
		import cetz.draw: *
		set-style(
...
$ arrow(E) &=
  ...
  &= frac(2 eta z, 4 epsilon_(0)) (frac(1, abs(z)) - frac(1, sqrt(z^(2) + R^(2)))) hat(k) =
  frac(2 eta, 4 epsilon_(0)) (frac(z, abs(z)) - frac(z, sqrt(z^(2) + R^(2)))) hat(k) =
  frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (frac(z, abs(z)) - frac(z, sqrt(z^(2) + R^(2)))) hat(k) $

To study the strength of the electric field with respect to the distance,
it is first necessary to compute the magnitude, which is the absolute
value of the $z$ component:

$ E &= abs(frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (frac(z, abs(z)) - frac(z, sqrt(z^(2) + R^(2))))) =
  frac(1, 4 pi epsilon_(0)) abs(frac(2 Q, R^(2)) (frac(z, abs(z)) - frac(z, sqrt(z^(2) + R^(2))))) = \
  &= frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2)) abs(frac(z, abs(z)) - frac(z, sqrt(z^(2) + R^(2)))) =
  frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2)) abs(frac(z sqrt(z^(2) + R^(2)) - abs(z) z, abs(z) sqrt(z^(2) + R^(2)))) = \
  &= frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2)) frac(abs(z (sqrt(z^(2) + R^(2)) - abs(z))), abs(abs(z) sqrt(z^(2) + R^(2)))) =
  frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2)) frac(cancel(abs(z)) abs(sqrt(z^(2) + R^(2)) - abs(z)), cancel(abs(z)) sqrt(z^(2) + R^(2))) = \
  &= frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2)) frac(sqrt(z^(2) + R^(2)) - abs(z), sqrt(z^(2) + R^(2))) =
  frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2)) (1 - frac(abs(z), sqrt(z^(2) + R^(2)))) $

The derivative test cannot be used to find a maximum, since $(d slash d z)
E = 0$ has no solution #footnote[Not shown for brevity.]. However, $abs(z)
slash sqrt(z^(2) + R^(2))$ is always proper ratio, since $abs(z) lt.eq
sqrt(z^(2) + R^(2))$ for any $z$. Therefore, an "educated guess" for a
maxima of the function would be at $z = 0$:

$ f(0) gt.eq f(z) &=>
  cancel(frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2))) (1 - frac(abs(0), sqrt(0^(2) + R^(2)))) gt.eq
  cancel(frac(1, 4 pi epsilon_(0)) frac(2 abs(Q), R^(2))) (1 - frac(abs(z), sqrt(z^(2) + R^(2)))) \
  &=> cancel(1) - frac(0, sqrt(0 + R^(2))) gt.eq cancel(1) - frac(abs(z), sqrt(z^(2) + R^(2))) =>
  0 gt.eq -frac(abs(z), sqrt(z^(2) + R^(2))) =>
  frac(abs(z), sqrt(z^(2) + R^(2))) gt.eq 0 $

Which holds for any $z$, since $abs(z) slash sqrt(z^(2) + R^(2))$ is
always positive.

#figure(
	caption: [Strength of the electric field with respect to the value of the $z$ coordinate.],
	[#image("disk.svg", width: 75%)]
)
...
$ arrow(E) &=
  frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (frac(z, abs(z)) - frac(z, sqrt(z^(2) + R^(2)))) hat(k) =
  frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (frac(cancel(z), cancel(z)) - frac(z, sqrt(z^(2)(1 + frac(R^(2), z^(2)))))) hat(k) = \
  &= frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (1 - frac(cancel(z), cancel(z) sqrt((1 + frac(R^(2), z^(2)))))) hat(k) = 
  frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (1 - (1 + frac(R^(2), z^(2)))^(-frac(1, 2))) hat(k) $

Applying the binomial approximation:

$ arrow(E) &=
  frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (1 - (1 + frac(R^(2), z^(2)))^(-frac(1, 2))) hat(k) approx
  frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (1 - (1 - frac(1, 2) frac(R^(2), z^(2)))) hat(k) = \
  &= frac(1, 4 pi epsilon_(0)) frac(2 Q, R^(2)) (cancel(1) - cancel(1) + frac(R^(2), 2 z^(2))) hat(k) =
  frac(1, 4 pi epsilon_(0)) frac(cancel(2) Q, cancel(R^(2))) frac(cancel(R^(2)), cancel(2) z^(2)) hat(k) =
