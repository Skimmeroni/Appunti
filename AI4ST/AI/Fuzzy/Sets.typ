...
$h(mu) = 1$, otherwise it is said to be *subnormal*. Subnormal fuzzy
sets possess no members having complete set membership.

It is possible to extend the set operations of union, intersection and
complement to fuzzy sets in the following way:
...
To give the rationale behind this definition, it is necessary to introduce
fuzzy logic.
