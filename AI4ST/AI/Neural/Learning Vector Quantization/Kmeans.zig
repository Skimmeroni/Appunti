...
pub fn kmeans(clusteringResult: []usize, dataset: *Matrix, numberOfClusters: usize) ! void {
    ...
    var centroids = try Matrix.init(allocator, numberOfClusters, dataset.columns, 0);
    defer centroids.deinit();

    var newCentroids = try Matrix.init(allocator, numberOfClusters, dataset.columns, 0);
    defer newCentroids.deinit();

    var distsFromCentroids = try allocator.alloc(f32, numberOfClusters);
    defer allocator.free(distsFromCentroids);

    var clusterSizes = try allocator.alloc(usize, numberOfClusters);
    defer allocator.free(clusterSizes);

    @memset(distsFromCentroids[0..], 0);
    @memset(clusterSizes[0..], 0);

    for (0..centroids.rows) |centroid| {
        const index = std.crypto.random.uintLessThan(usize, dataset.rows);
        @memcpy(centroids.entry(centroid), dataset.entry(index));
    }

    for (0..NUMBER_OF_ITERATIONS) |_| {
        for (0..dataset.rows) |i| {
            for (0..numberOfClusters) |j| {
                distsFromCentroids[j] = euclideanDistance(centroids.entry(j), dataset.entry(i));
            }
            const chosenCluster = std.sort.argMin(f32, distsFromCentroids, {}, std.sort.asc(f32)).?; // Huh?
            clusterSizes[chosenCluster] += 1;

            for (0..newCentroids.columns) |j| {
                newCentroids.entry(chosenCluster)[j] += dataset.entry(i)[j];
            }
        }

        for (0..newCentroids.rows) |i| {
            for (0..newCentroids.columns) |j| {
                newCentroids.entry(i)[j] /= @floatFromInt(clusterSizes[i]);
            }
        }

        @memcpy(centroids.data, newCentroids.data);
        @memset(newCentroids.data[0..], 0);
        @memset(clusterSizes[0..], 0);
    }
