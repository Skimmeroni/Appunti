...
#algo(
	...
	parameters: ([$f : RR^(n) -> RR$: function, $nu$: integer, $epsilon$: termination criteria],),
	stroke: 0pt,
	indent-guides: 1pt + gray,
	fill: none,
	[
		$x = (x_(1), dots, x_(n)) <-$ #smallcaps("Random") ($n$) #comment[Randomly initialize a point] \
		do #i \
...
#algo(
	...
	[
		$x = (x_(1), dots, x_(n)) <-$ #smallcaps("Random") ($n$) #comment[Randomly initialize a point] \
		do #i \
			$x' = (x'_(1), dots, x'_(n)) <-$ #smallcaps("Random-Neighborhood") ($n, x$) #comment[Random point in the neighborhood] \
			if ($f(x') > f(x)$) #i \
...
#algo(
	...
	parameters: ([$f : RR^(n) -> RR$: function, $T$: float, $epsilon$: termination criteria],),
	stroke: 0pt,
	indent-guides: 1pt + gray,
	fill: none,
	[
		...
		do #i \
			...
			if ($Delta > 0$ or $p gt.eq$ #smallcaps("Random-value") $(0, 1)$) #i #comment[New solution is better or just tolerated] \
				$x <- x'$ #d \
			$T <- T - 0.1$ #d #comment[Decrease temperature] \
		while (not($epsilon$)) \
		return $x$ #comment[Return best solution found] \
	]
)
	...
	[
		...
		do #i \
			...
			if ($f(x) - f(x') < theta$) #i #comment[New solution is better or just tolerated] \
				$x <- x'$ #d \
			$theta <- theta - 0.1$ #d #comment[Decrease threshold] \
		while (not($epsilon$)) \
		return $x$ #comment[Return best solution found] \
	]
)
