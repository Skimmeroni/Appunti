...
leggero e deformabile.] e _temperatura_. Tutte e tre si dicono
*variabili di stato*, perché il loro valore dipende dalla configurazione
(dallo stato, appunto) in cui si trovano i loro atomi e molecole.

Una equazione in grado di mettere in relazione le variabili di
stato di un gas prende il nome di *equazione di stato*. Al fine
di costruire una equazione di questo tipo, é conveniente semplificare
il modello di un gas di modo da renderlo approcciabile. Inoltre, una
proprietá attesa del modello é che non descriva solamente un gas nello
specifico (idrogeno, ossigeno, elio, ecc...), ma che sia applicabile
a qualsiasi gas.
