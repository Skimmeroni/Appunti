...
#theorem("Condizione necessaria e sufficiente per la solubilitá delle equazioni diofantee")[
	...
	in ZZ$ e incognite $x, y in ZZ$. Tale equazione ammette soluzione se e soltanto se
	$"MCD"(a, b) | c$.
] <Diophantine-solutions-exist>
#proof[
	Si supponga che $a x + b y = c$ ammetta una certa soluzione $(x_(0), y_(0)) in ZZ
	times ZZ$. Deve allora valere $a x_(0) + b y_(0) = c$. Valendo $"MCD"(a, b) | a
	...
	per un qualche $tilde(c) in ZZ$. Per l'identità di Bezout esistono certi
	$s, t in ZZ$ tali per cui $"MCD"(a, b) = a s + b t$. Sostituendo nell'equazione
	...
	$(x_(0) , y_(0)) in ZZ times ZZ$, tale coppia é una possibile soluzione per
...
#proof[
	...
	ambo i membri per un certo $tilde(c) in ZZ$, si ha $(a x + b y) tilde(c) =
...
#theorem("Soluzioni di una equazione diofantea")[
	...
	nulli $a, b, c in ZZ$ e incognite $x, y in ZZ$. Se la coppia $(x_(0), y_(0))
	in ZZ times ZZ$ é soluzione per tale equazione, allora lo sono tutte e sole le
	coppie $(x_(h), y_(h)) in ZZ times ZZ$ cosí costruite:
...
#proof[
	...
	ZZ$ tale per cui $y_(0) − overline(y) = h overline(a)$, cioé $overline(y) =
	...
	Essendo $(overline(x), overline(y)) in ZZ times ZZ$ una soluzione generica,
...
#example[
	...
	famiglia di soluzioni $(x_(h), y_(h)) in ZZ times ZZ$:
